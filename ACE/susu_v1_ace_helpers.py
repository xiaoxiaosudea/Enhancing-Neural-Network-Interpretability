""" collection of various helper functions for running susu_pth_ACE"""
...
def load_image_from_file(filename, shape):
  ...
  if not tf.gfile.Exists(filename):
    ...
    return None
  try:
    img = np.array(Image.open(filename).convert("RGB").resize(
...
def plot_concepts(cd, bn, num=40, address=None, mode='diverse', concepts=None):
...
def save_concepts(cd, concepts_dir):
  ...
  for bn in cd.bottlenecks:
    for concept in cd.dic[bn]['concepts']:
      ...
      patches = (np.clip(cd.dic[bn][concept]['patches'], 0, 1) * 255).astype(
          np.uint8)
      images = (np.clip(cd.dic[bn][concept]['images'], 0, 1) * 255).astype(
          np.uint8)
